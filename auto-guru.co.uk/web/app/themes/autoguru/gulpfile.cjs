...
gulp.task('serve', function () {
  browserSync.init({
    proxy: 'http://localhost', // Replace with your local development URL
    notify: false,
  });
  gulp.watch(paths.css.src, gulp.series('build-css')); // Watch for changes in CSS files
  gulp.watch(paths.twig.src).on('change', browserSync.reload); // Reload browser on Twig changes
});
